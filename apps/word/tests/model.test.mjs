...
test("single image is treated as expected result when result image is missing", () => {
  ...
  assert.deepEqual(Array.from(visuals.resultImages), [{ src: "https://img/ex1.jpg", caption: "" }]);
});
...
test("getExerciseStepsView separates introductory sentence from actionable steps", () => {
    ...
    exercises: [
      {
        ...
        instructions: [
          ...
          "Présentez le document sur deux feuilles côte à côte.",
        ],
      },
    ],
  });
  ...
  assert.equal(stepsVm.preamble, "Petit exercice de prise en main pour sortir de la présentation classique.");
